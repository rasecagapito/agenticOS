...
## Porquê
Utilizadores pedem tema escuro; reduz fadiga visual e alinha com concorrência.

## Escopo
- Toggle claro/escuro no header
- Persistência da preferência (localStorage)
- NÃO entra: temas customizados pelo utilizador
...
**3. `/wrapup`** deteta mudança concluída → arquiva + sugere delta:
...
`changes/archive/2026-06-11-add-dark-mode/`. Registo em `memory/history/` + `memory/learnings/`.
...
2. **Plano de reorganização** — gera `changes/reorganize-<alvo>/proposal.md` + `tasks.md`
   ...
   - [ ] AGRUPAR notas-*.md (3 ficheiros) → docs/notas/            (dispersos na raiz)
   - [ ] MARCAR  config.bak.json        → redundante (idêntico a config.json)
   ```
3. **Aprovação humana** — utilizador confirma o plano inteiro ou item a item.
...
- **Backup do que se move.** Registar o mapa origem→destino em
  `changes/reorganize-<alvo>/MOVES.md` para rollback manual.
- **Dry-run por defeito.** O scan e o plano nunca alteram nada; só `/worker` após aprovação move.
...
- **`AGENTIC-OS.md`** de cada template — secção curta que **referencia** o doc acima (não duplica).
...
| Alvo | Alteração |
|------|-----------|
| `template/A-generico/` | pasta `changes/` + `changes/archive/` (.gitkeep); `.claude/commands/propose.md`; atualizar wrapup.md + worker.md; CLAUDE.md ganha `/propose` na tabela; AGENTIC-OS.md secção |
...
2. `/worker developer` deteta e carrega a mudança ativa.
