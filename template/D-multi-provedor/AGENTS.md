...
> Cérebro canónico — orquestrador central partilhado por TODAS as IAs (Claude, Codex, Gemini, GLM, DeepSeek…).
> Mantém 100-200 linhas. Este é o único ficheiro de orquestração; os outros (`CLAUDE.md`, `GEMINI.md`) apenas apontam para aqui.
...
3. Nunca deletar/mover ficheiros sem confirmação humana.
...
No Claude há slash commands (`/propose`, `/worker`, `/wrapup`, `/status`, `/handoff`) que os invocam.
...
## Provedores
Ficheiro de entrada por IA e limitações em `providers/registry.md`.
Este `AGENTS.md` é o cérebro canónico. `CLAUDE.md` e `GEMINI.md` são ponteiros (`@`/import) para aqui — **não duplicar conteúdo neles**.
