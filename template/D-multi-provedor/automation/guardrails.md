...
- Deletar ou mover ficheiros
- Alterar o ficheiro cérebro canónico (`AGENTS.md`) em estrutura (não só "Estado do Projeto")
...
- **Fonte única**: só `AGENTS.md` (ou o cérebro detetado) orquestra. `CLAUDE.md`/`GEMINI.md` são ponteiros `@import` — nunca colar conteúdo neles.
- **Carimbo de provedor**: toda a gravação de handoff regista qual IA a fez + hora real.
...
- **Antes de trocar de IA**: garantir handoff atualizado; se uma tarefa ficou a meio, registar em "Gotchas".
- **Credenciais**: sempre via variáveis de ambiente, nunca em `context/`, `memory/` ou nos ficheiros cérebro.
