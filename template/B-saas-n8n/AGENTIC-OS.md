...
| Comando | O que faz |
|---------|-----------|
| `/wrapup` | Consolida sessão → registro em memory/history + aprendizados em memory/learnings |
| `/status` | Branch, últimos commits, features, próximos passos |
| `/worker dev` | Ativa developer — React/Supabase/TypeScript |
| `/worker workflow` | Ativa workflow designer — n8n, WhatsApp, Telegram |
| `/worker arquiteto` | Ativa arquiteto — decisões técnicas, migrations |
| `/worker qa` | Ativa revisor — segurança, testes, Notion flow |
...
3. /worker [nome] → ativa especialista com contexto SaaS/n8n
4. [trabalho da sessão]
5. /wrapup → consolida sessão + atualiza context/estado-sistema
...
| `workflow_n8n` | Antes de ativar workflow em produção |
